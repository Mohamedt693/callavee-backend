...
const scheduleTask = (cronExpression) => {
    ...
    console.log(`[Cron Job] Task scheduled: ${cronExpression}`);
};

export const initScraper = async () => {
    let config = await Scraper.findOne({ jobName: 'price-update' });
    if (!config) config = await Scraper.create({}); 
    if (config.isActive) scheduleTask(config.cronExpression);
};
...
export const updateScraperSettings = async (req, res) => {
    try {
        ...
        if (cronExpression && !cron.validate(cronExpression)) {
            return res.status(400).json({ success: false, message: "Invalid cron format." });
        }
...
export const getScraperSettings = async (req, res) => {
    try {
        const config = await Scraper.findOne({ jobName: 'price-update' }); // Fixed the model name
